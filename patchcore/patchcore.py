...
class PatchCore:
    ...
    def calc_distance(self, embedding, n_neighbors):
        # Compute nearest neighbor point and it's score (L2 distance).
        distances, _ = self.searcher.predict(embedding, k=n_neighbors)
        return distances

    def make_anomaly_map(self, distances, x):
        # Compute anomaly map and it's re-weighting.
        anomaly_map_rw, _ = self.compute_anomaly_scores(distances, x.shape)
    ...
    def compute_anomaly_scores(seld, score_patches, x_shape):
        ...
        anomaly_maps = [cv.resize(amap, (x_shape[3], x_shape[2]), interpolation=cv.INTER_CUBIC) for amap in anomaly_maps]
