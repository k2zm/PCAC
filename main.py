...
from patchcore.dataset    import MVTecADImageOnly, MVTecAD
from patchcore.patchcore  import PatchCore
from patchcore.utils      import auto_threshold
from patchcore.clustering import Clustering
...
def main(args):
    ...
    if args.mode == "train":
        ...
        # Prepare dataset.
        dataset = MVTecAD(args.input, split="train", **dataset_args)
        ...
        for x, gt, label, filepath, x_type in rich.progress.track(dloader, description="Processing..."):
            ...
            distances = model.calc_distance(embedding, args.n_neighbors)

            embwa = clustering.weight_average(embedding, distances)
            clustering.append_embwa(embwa, x_type)

            anomaly_map_rw = model.make_anomaly_map(distances, x)
